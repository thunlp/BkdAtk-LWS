...
def static_poisoning_sst2_bert(poison_rate_train, config, rule):
    ...
    import sys
    model_biclassifier = DistilBertForSequenceClassification.from_pretrained(sys.argv[1]).cuda()
    ...
    def depoison(dataset):
        sentences = []
        for i in dataset:
            [sent, label] = i
            sentences.append(sent)
        all_ppl = get_PPL(sentences)
        all_depoisoned = get_processed_poison_data(all_ppl, sentences, bar)
        return [[all_depoisoned[x], dataset[x][1]] for x in range(len(dataset))]
    #dev1 = depoison(dev1)
    test1 = test1[:200]
    test_original = test_original[:200]
    test1 = depoison(test1)
    test_original = depoison(test_original)
    ...
    # Train model
    bert_biclassifier.train(model_biclassifier, criterion, opti, train_loader, [dev_loader, dev1_loader, dev2_loader], [val_loader, test_loader, test2_loader], argv, max_eps=0, gpu=0)
